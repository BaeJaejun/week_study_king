...
def login():
    if request.method == "POST":
        ...
        if user and check_password_hash(user["password"], password):
            ...
            refresh_payload = {
                'id': userid,
                'exp': datetime.utcnow() + timedelta(days=7)  # refresh토큰 만료 시간을 7일으로 설정
            }
...
def refresh():
    ...
    try:
        ...
        try:
            payload = jwt.decode(client_refresh_token, jwt_secret_key_refresh, algorithms=["HS256"], options={"verify_exp": False})
            userid = payload.get("id")
